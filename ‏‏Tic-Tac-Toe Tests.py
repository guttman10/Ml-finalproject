...
def MonteCarloTreeSearch (g,curr_state,move_list,x):
    best_move = 0
    second_best = 0
    move_index=0
    val_list = []
    ...
    for x in range(1,6):
        ...
        for i in range(0,100):
            ...
            while(g.check_winner(g, state) == 0):
                ...
                if( g.check_winner(g, state) == -1 ):
                    tie_score +=1
            print("Winner ",g.check_winner(g, state))

        f.write("Win %"+str(p1_score/100)+" ")
        f.write("Lose %"+str(p2_score/100)+" ")
        f.write("Tie %"+str(tie_score/100)+"\n")
